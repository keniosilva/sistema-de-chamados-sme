...
// Proteção: só técnicos de informática podem acessar
verificarPermissao(['tecnico_informatica']);
...
$stmt = $conn->prepare("SELECT status, COUNT(*) as total FROM chamados WHERE id_tecnico_responsavel = ? GROUP BY status");
if ($stmt) {
...
$stmt = $conn->prepare("SELECT tipo_manutencao, COUNT(*) as total FROM chamados WHERE id_tecnico_responsavel = ? GROUP BY tipo_manutencao");
if ($stmt) {
    $stmt->bind_param("i", $id_tecnico);
    $stmt->execute();
    $result = $stmt->get_result();
...
$stmt = $conn->prepare("SELECT DATE(data_abertura) as dia, COUNT(*) as total FROM chamados WHERE id_tecnico_responsavel = ? AND data_abertura >= DATE_SUB(NOW(), INTERVAL 30 DAY) GROUP BY dia");
if ($stmt) {
    $stmt->bind_param("i", $id_tecnico);
    $stmt->execute();
    $result = $stmt->get_result();
...
// Consulta principal com JOINs e filtros
$conditions = ["c.id_tecnico_responsavel = ?"];
$params = [$id_tecnico];
$types = 'i';
...
<head>
    ...
    <title>Dashboard - Técnico de Informática</title>
            ...
            <div class="col-12 col-lg-10">
                <div class="card shadow-sm mb-4">
                    <div class="card-header bg-primary text-white">
                        <h2 class="mb-0"><i class="bi bi-tools"></i> Dashboard Técnico de Informática - Meus Chamados</h2>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover table-bordered align-middle">
                                ...
                                <tbody>
                                    ...
                                    <?php else: ?>
                                        <tr>
                                            <td colspan="8" class="text-center">Nenhum chamado atribuído a você foi encontrado.</td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        ...
                        <div class="card shadow-sm mt-4">
                            ...
                            <div class="card-body">
                                <p>Total de chamados nos últimos 30 dias: 
                                    <?php
                                    $stmt = $conn->prepare("SELECT COUNT(*) as total FROM chamados WHERE id_tecnico_responsavel = ? AND data_abertura >= DATE_SUB(NOW(), INTERVAL 30 DAY)");
                                    $stmt->bind_param("i", $id_tecnico);
                                    $stmt->execute();
